...
(function () {
  ...
  function renderTelemetry(m) {
    ...
    $('#loadFill').style.background = over ? 'linear-gradient(90deg,#ff5d6c,#ff9f43)' : 'linear-gradient(90deg,#c8f542,#3dd68c)';
  ...
  function drawArrival() {
    ...
    ctx.shadowColor = '#c8f542'; ctx.shadowBlur = 7; ctx.stroke(); ctx.shadowBlur = 0;
  }
  ...
  function drawEnergySpark() {
    ...
    ctx.beginPath();
    vals.forEach((v, i) => (i === 0 ? ctx.moveTo(i * xStep, yOf(v)) : ctx.lineTo(i * xStep, yOf(v))));
    ctx.strokeStyle = '#3dd68c'; ctx.lineWidth = 1.6; ctx.stroke();
  }
