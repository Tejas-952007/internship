...
(function () {
  ...
  class Gauge {
    constructor(el, opts) {
      this.el = el;
      this.o = Object.assign({ min: 0, max: 100, format: v => v.toFixed(0), color: () => '#c8f542' }, opts);
      this.value = this.o.min;

      const w = 120, h = 64, cx = 60, cy = 58, r = 48;
      el.innerHTML =
        `<svg viewBox="0 0 ${w} ${h}">
           <path class="g-track" d="${arcPath(cx, cy, r, -120, 120)}" fill="none" stroke="rgba(232,220,196,.14)" stroke-width="7" stroke-linecap="round"/>
           <path class="g-fill" d="" fill="none" stroke="#c8f542" stroke-width="7" stroke-linecap="round"/>
           <line class="g-needle" x1="0" y1="0" x2="0" y2="0" stroke="#f4efe4" stroke-width="2" stroke-linecap="round"/>
           <circle class="g-hub" cx="${cx}" cy="${cy}" r="4" fill="#c8f542"/>
         </svg>
